...
const DreamsSection = () => {
  ...
  return (
    <section
      ref={sectionRef}
      aria-label="Her aspirations"
      className="min-h-[100dvh] py-20 sm:py-24 md:py-28 px-4 sm:px-6 flex items-center bg-background"
    >
      <div className="max-w-5xl mx-auto w-full">
        <h2 className="font-serif text-2xl sm:text-3xl md:text-4xl text-center text-foreground/90 mb-4 sm:mb-6">
          Things She Moves Toward
        </h2>
        <p className="text-center text-foreground/60 text-sm sm:text-base mb-10 sm:mb-12 md:mb-16 max-w-md mx-auto px-4">
          Not destinations — just directions that feel right.
        </p>

        {/* Dreams Grid - Staggered */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-5 lg:gap-6 mb-10 sm:mb-12" role="list">
          {dreams.map((dream, index) => {
            ...
            return (
              ...
              >
                <div className={`relative bg-background rounded-xl sm:rounded-2xl ${isTall ? 'p-5 sm:p-6 md:p-7 lg:p-9' : 'p-4 sm:p-5 md:p-6 lg:p-8'} border border-border/30 hover:border-border/50 transition-all duration-500 text-center`}>
                  <div className="relative z-10">
                    <div className="w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 mx-auto mb-3 sm:mb-4 md:mb-5 rounded-lg sm:rounded-xl bg-accent/40 flex items-center justify-center group-hover:-translate-y-1 transition-transform duration-500 ease-out" aria-hidden="true">
                      <Icon className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-primary" />
                    </div>
                    <p className="font-sans text-foreground/80 text-xs sm:text-sm md:text-base leading-relaxed group-hover:text-foreground transition-colors duration-500">
                      {dream.label}
                    </p>
                  </div>
                </div>
